...
public class UbahPaketTour extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_ubah_paket_tour);

        Firebase.setAndroidContext(this);
        FirebaseApp.initializeApp(UbahPaketTour.this);
        ...
        etNama.setText(paketTour.getNamaPaket());
        etHarga.setText(paketTour.getHargaPaket());
        etDurasi.setText(paketTour.getDurasiPaket());
        etJmlPeserta.setText(paketTour.getJumlahPeserta());
        etFasilitas.setText(paketTour.getFasilitasPaket());
        Glide.with(this)
        ...
        imgBrowse.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (ContextCompat.checkSelfPermission(getApplicationContext(), android.Manifest.permission.READ_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
                    ActivityCompat.requestPermissions(UbahPaketTour.this, new String[] {android.Manifest.permission.READ_EXTERNAL_STORAGE}, RC_PERMISSION_READ_EXTERNAL_STORAGE);
                } else {
    ...
    private void uploadTanpaGambar(){
        ref.child("pakettour").child(SharedVariable.paket).child(paketTour.getKey()).child("namaPaket").setValue(etNama.getText().toString());
        ref.child("pakettour").child(SharedVariable.paket).child(paketTour.getKey()).child("hargaPaket").setValue(etHarga.getText().toString());
        ref.child("pakettour").child(SharedVariable.paket).child(paketTour.getKey()).child("durasiPaket").setValue(etDurasi.getText().toString());
        ref.child("pakettour").child(SharedVariable.paket).child(paketTour.getKey()).child("fasilitasPaket").setValue(etFasilitas.getText().toString());

        new SweetAlertDialog(UbahPaketTour.this, SweetAlertDialog.SUCCESS_TYPE)
                .setTitleText("Sukses!")
    ...
    private void uploadData(final Uri uri){
        ...
        uploadTask.addOnFailureListener(new OnFailureListener() {
            ...
            public void onFailure(@NonNull Exception exception) {
                ...
                Toast.makeText(UbahPaketTour.this, "Upload failed!\n" + exception.getMessage(), Toast.LENGTH_LONG).show();
                pDialogLoading.dismiss();
            }
            ...
            public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                ...
                pDialogLoading.dismiss();
                Toast.makeText(UbahPaketTour.this, "Upload gambar finished!", Toast.LENGTH_SHORT).show();
                ...
                ref.child("pakettour").child(SharedVariable.paket).child(paketTour.getKey()).child("durasiPaket").setValue(etDurasi.getText().toString());
                ref.child("pakettour").child(SharedVariable.paket).child(paketTour.getKey()).child("fasilitasPaket").setValue(etFasilitas.getText().toString());
                ref.child("pakettour").child(SharedVariable.paket).child(paketTour.getKey()).child("downloadUrl").setValue(downloadUrl.toString());
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == RC_IMAGE_GALLERY && resultCode == RESULT_OK) {
            uri = data.getData();
            final String tipe = GetMimeType(UbahPaketTour.this,uri);
            Toast.makeText(UbahPaketTour.this, "Tipe : !\n" + tipe, Toast.LENGTH_LONG).show();
